...
export default function FileDetailPage({ result }: Props) {
  ...
  const hotspots = new Map<number, { color: string; border: string }>();
  file.code_smells.forEach(s => {
    if (s.line) hotspots.set(s.line, { color: 'rgba(239, 68, 68, 0.15)', border: '#ef4444' });
  });
  file.functions.forEach(fn => {
    if (fn.complexity > 10) {
      for (let i = fn.line_start; i <= (fn.line_end || fn.line_start); i++) {
        if (!hotspots.get(i)) hotspots.set(i, { color: 'rgba(210, 193, 182, 0.05)', border: 'var(--accent)' });
      }
    }
  });
      ...
      <div style={{ position: 'relative', minHeight: '100vh', paddingBottom: 60 }}>
        ...
        <div className="responsive-container" style={{ position: 'relative', zIndex: 1, maxWidth: 1100, paddingTop: 32, paddingBottom: 32 }}>
          ...
          <div className="file-header">
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>File Report</div>
              <h1 className="title truncate" style={{ margin: 0, fontSize: '1.5rem', fontWeight: 900 }}>
                {file.file_path}
              </h1>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 14 }}>
            <MetricCard label="Complexity Index" value={file.cyclomatic_complexity.toFixed(1)} color={file.cyclomatic_complexity > 15 ? '#ef4444' : '#D2C1B6'} />
            <MetricCard label="Cognitive Load" value={file.cognitive_complexity.toFixed(1)} color={file.cognitive_complexity > 20 ? '#f59e0b' : '#D2C1B6'} />
            <MetricCard label="Maintainability" value={`${file.maintainability_index}%`} color={file.maintainability_index >= 60 ? '#10b981' : '#f59e0b'} />
            <MetricCard label="Bug Risk" value={`${Math.round(file.bug_risk_probability * 100)}%`} color={file.bug_risk_probability > 0.5 ? '#ef4444' : '#D2C1B6'} />
          </div>
          ...
          {file.code_content && (
            <div style={{ ...card, marginBottom: 24, padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(210,193,182,0.02)' }}>
                <div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 800 }}>Hotspot Analysis</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>{hotspots.size} high-risk lines identified</div>
                </div>
              </div>
              <div style={{ maxHeight: 650, overflowY: 'auto' }}>
                <SyntaxHighlighter
                  ...
                  lineProps={(lineNumber) => {
                    const hotspot = hotspots.get(lineNumber);
                    if (hotspot) {
                      return {
                        style: {
                          display: 'block',
                          backgroundColor: hotspot.color,
                          borderLeft: `3px solid ${hotspot.border}`,
                          width: '100%',
                        }
                      };
                    }
                  ...
                  lineNumberStyle={{ color: 'rgba(210,193,182,0.2)', minWidth: '4em', paddingRight: '1.5em', textAlign: 'right' }}
                >
                  {file.code_content}
                </SyntaxHighlighter>
              </div>
